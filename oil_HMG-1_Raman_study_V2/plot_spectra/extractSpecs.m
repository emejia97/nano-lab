function [wavenums, specs] = extractSpecs(fp, lambda_ex, N)
%calcERSR6G -- Summary --
...
for i = 1:N
    ...
    s = sData(i).d;
    Ns(i) = size(s.data,1); % # of samples for each measurement
...
for i = 1:N
    for j = 1:Ns(i)
        ...
        minRaw = min(s1ERS(i).data(j,:));
        ERS_sub = s1ERS(i).data(j,:) - minRaw;

        ERS_avg = mean(ERS_sub(ERS_index_range));

        specs(i).data(j,:) = specs(i).data(j,:) ./ ERS_avg; % ERS calibration
...
ERS = zeros([N, 1]); % ERS measurements and samples for each measurement
HMB1 = zeros([N, 1]); % R6G measurements and samples for each measurement
% Row = measurement
